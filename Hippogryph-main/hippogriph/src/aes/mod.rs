...
use self::{aes_utils::{key_expansion, pretty_print_nibbles, u8_to_vec_bool, vec_bool_to_u8}, casts::{decomposer, recomposer}, clear::clear_mixcolumns, linear_circuit::LinearCircuit};
...
pub fn run_aes(state: &AESStateBoolean, server_key:&ServerKey, aes_key : Vec<bool>, client_key_debug : &ClientKey) -> AESStateBoolean{
    ...
    // Key Expansion
    println!("TIMING PRE_KEY_EXPANSION {:?}",SystemTime::now().duration_since(UNIX_EPOCH).unwrap());
    let round_keys = key_expansion(aes_key);
    println!("TIMING POST_KEY_EXPANSION {:?}", SystemTime::now().duration_since(UNIX_EPOCH).unwrap());
    ...
    println!("TIMING START_ROUND {} {:?}", 10, SystemTime::now().duration_since(UNIX_EPOCH).unwrap());
    println!("Round {}", 10);
    state_arith = sub_bytes(&state_arith, server_key, client_key_debug);
    println!("TIMING POST_SUB_BYTES {} {:?}", 10, SystemTime::now().duration_since(UNIX_EPOCH).unwrap());

    
    state_bool = state_arith.aes_decomposer(&server_key, &client_key_debug);
    println!("TIMING POST_BOOLEAN_DECOMPOSITION {} {:?}", 10, SystemTime::now().duration_since(UNIX_EPOCH).unwrap());
    print_debug_arith(&state_arith, "");

    state_bool = shift_rows(&state_bool);
    println!("TIMING POST_SHIFT_ROWS {} {:?}", 10, SystemTime::now().duration_since(UNIX_EPOCH).unwrap());

    state_bool = add_round_key(&state_bool, &round_keys[10], server_key);
    println!("TIMING POST_ADD_ROUND_KEYS {} {:?}", 10, SystemTime::now().duration_since(UNIX_EPOCH).unwrap());
...
pub fn  demo_aes(){
    ...
    println!("TIMING PRE_TFHE_ENCRYPTION {:?}",  SystemTime::now().duration_since(UNIX_EPOCH).unwrap());

    let state = AESStateBoolean::tfhe_encryption_bits(&plaintext_bits, &client_key);
    println!("TIMING POST_TFHE_ENCRYPTION {:?}", SystemTime::now().duration_since(UNIX_EPOCH).unwrap());
    ...
    println!("TIMING PRE_TFHE_DENCRYPTION {:?}",  SystemTime::now().duration_since(UNIX_EPOCH).unwrap());

    let result_clear = result.tfhe_decryption_bits(&client_key);
    println!("TIMING POST_TFHE_DENCRYPTION {:?}",  SystemTime::now().duration_since(UNIX_EPOCH).unwrap());
