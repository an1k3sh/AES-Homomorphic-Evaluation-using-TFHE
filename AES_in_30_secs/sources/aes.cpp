...
PROFILE_MEMORY();  
  ...
  double elapsed;
  for(r = 1; r < ROUNDS; r++) {
    printf("Round %d start", r);
    clock_gettime(CLOCK_REALTIME, &ts);
    printf(": %ld.%ld\n", ts.tv_sec, ts.tv_nsec);
    ShiftRows_fhe(a, 0, 4, gk);
    clock_gettime(CLOCK_REALTIME, &ts);
    printf("After Shift Row: %ld.%ld\n", ts.tv_sec, ts.tv_nsec);
    MixColumns_fhe(a, gk, ks_key);
    clock_gettime(CLOCK_REALTIME, &ts);
    printf("After MixCol: %ld.%ld\n", ts.tv_sec, ts.tv_nsec);
    AddRoundKey_nextSubBytes_fhe(a, rk[r], gk, ks_key);
    clock_gettime(CLOCK_REALTIME, &ts);
    printf("After AddRoundKey: %ld.%ld\n", ts.tv_sec, ts.tv_nsec);
    printf("Round %d end", r);
    clock_gettime(CLOCK_REALTIME, &ts);
    printf(": %ld.%ld\n", ts.tv_sec, ts.tv_nsec);
  }
   ...
   */
  clock_gettime(CLOCK_REALTIME, &ts);
  printf("Last round: %ld.%ld\n", ts.tv_sec, ts.tv_nsec);
  ShiftRows_fhe(a, 0, 4, gk);
  clock_gettime(CLOCK_REALTIME, &ts);
  printf("Last round Shift Row: %ld.%ld\n", ts.tv_sec, ts.tv_nsec);
  AddRoundKey_fhe(a, rk[ROUNDS], gk, ks_key);
  clock_gettime(CLOCK_REALTIME, &ts);
  printf("Last round Add Round Key: %ld.%ld\n", ts.tv_sec, ts.tv_nsec);
  return 0;
}
